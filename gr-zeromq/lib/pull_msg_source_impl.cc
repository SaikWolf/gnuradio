...
pull_msg_source::sptr pull_msg_source::make(char* address, int timeout, int linger, bool bind)
{
    return gnuradio::make_block_sptr<pull_msg_source_impl>(address, timeout, linger, bind);
}

pull_msg_source_impl::pull_msg_source_impl(char* address, int timeout, int linger, bool bind)
...
void pull_msg_source_impl::teardown()
{
    if (d_terminated == 0){
        stop();
        d_context.shutdown();
